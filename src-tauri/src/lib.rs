...
async fn fetch_scoreboard() -> Result<Value, String> {
    let client = reqwest::Client::builder().timeout(Duration::from_secs(15)).user_agent("OnlyBeats-Command-Center/0.7.0").build().map_err(|e| e.to_string())?;
...
async fn fetch_weather(input: WeatherInput) -> Result<Value, String> {
    let client = reqwest::Client::builder()
        ...
        .user_agent("OnlyBeats-Command-Center/0.7.0")
