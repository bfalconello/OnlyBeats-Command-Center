...
async function syncScores(silent=false){
  ...
  try{
    ...
    if(requestId===refreshRequestId){
      ...
      if(['wall','dashboard','briefing','watch','schedule','rankings','news','favorites','teams','developer'].includes(currentPage))renderPage();
...
function renderPageUnsafe(){
  ...
  $('content').innerHTML=currentPage==='dashboard'?dashboard():currentPage==='briefing'?smartBriefingPage():currentPage==='wall'?wallPage():currentPage==='watch'?watchCenterPage():currentPage==='schedule'?schedulePage():currentPage==='favorites'?favoritesPage():currentPage==='teams'?teamHubPage():currentPage==='rankings'?intelligenceEnginePage():currentPage==='news'?newsPage():currentPage==='weather'?weatherPage():currentPage==='availability'?availabilityPage():currentPage==='predictions'?predictionsPage():currentPage==='reports'?reportsPage():currentPage==='developer'?developerPage():currentPage==='settings'?settingsPage():placeholderPage(currentPage,label);
  bindPage();
}
function renderPage(){
