...
function runProductionReleaseChecks(){
  ...
  const checks=[
    ...
    productionCheck('Favorites & Watchlists',typeof favoritesWatchlistsPage==='function','Unified favorites and watchlists'),
    ...
    productionCheck('Installed app updater',typeof installedAppUpdatesPage==='function'&&Boolean(window.onlyBeatsDesktop?.update),'GitHub release checks, download, and restart install'),
    productionCheck('Persistent Firebase configuration',Boolean(window.onlyBeatsDesktop?.saveFirebaseConfig)&&Boolean(window.ONLYBEATS_FIREBASE_CONFIG?.projectId),'User-data configuration survives installed updates'),
    productionCheck('Sandboxed preload bridge',Boolean(window.onlyBeatsDesktop?.getBridgeHealth)&&Boolean(window.onlyBeatsDesktop?.update),'Preload loads without package filesystem access'),
    ...
    productionCheck('Automatic update verification release',VERSION==='6.0.4'&&typeof installedAppUpdatesPage==='function','Visible post-update version marker and verification panel'),
    productionCheck('Mobile startup gate',typeof onlyBeatsDesktopRuntimeReady==='function'&&onlyBeatsDesktopRuntimeReady(),'Desktop runtime is optional for hosted PWA'),
